...
def main():
    ...
    target_model_class = MLD if (is_mld or is_lcm or is_controlnet) else VAE
    ...
    # load EEG control encoder
    eeg_ckpt_path = "experiments_eeg/eeg_control_encoder_v2.ckpt"
    eeg_control = None
    if osp.exists(eeg_ckpt_path):
        ckpt = torch.load(eeg_ckpt_path, map_location="cpu")
        in_ch = int(ckpt.get("in_ch", 27))
        hidden = int(ckpt.get("hidden", 128))
        latent_dim = int(ckpt.get("latent_dim", 32))
        out_dim = int(ckpt.get("out_dim", 768))
        normalize = bool(ckpt.get("normalize", True))

        eeg_control = EEGControlEncoder(
            in_ch=in_ch, hidden=hidden, latent_dim=latent_dim, out_dim=out_dim, normalize=normalize
        ).to(device).eval()

        sd = ckpt.get("eeg_control_encoder", None)
        if sd is None:
            sd = ckpt.get("eeg_encoder", None)
        if sd is None:
            raise KeyError(f"Cannot find state dict in {eeg_ckpt_path}")

        eeg_control.load_state_dict(sd, strict=True)
        eeg_control.requires_grad_(False)
        logger.info(f"Loaded EEG control encoder from {eeg_ckpt_path}")
    else:
        logger.info(f"EEG ckpt not found at {eeg_ckpt_path}, run text-only demo")
    ...
    if cfg.example is not None and not is_controlnet:
        ...
        # add EEG conditioning (only if encoder exists)
        if eeg_control is not None:
            ...
            for t in text:
                ...
                eeg = np.load(eeg_path).astype(np.float32)          # [T,C]
                eeg_list.append(torch.from_numpy(eeg))              # [T,C]

            eeg_batch = torch.stack(eeg_list, dim=0).to(device)     # [B,T,C]

            with torch.no_grad():
                motion_cond_emb, action_latent, eeg_feat = eeg_control(eeg_batch)  # [B,1,768], [B,d], [B,hidden]

            batch["eeg_emb"] = motion_cond_emb

            logger.info(f"Using EEG conditioning. Paths: {eeg_paths}")
            logger.info(f"motion_cond_emb shape: {tuple(motion_cond_emb.shape)} stored in batch['eeg_emb']")
            logger.info(f"action_latent shape: {tuple(action_latent.shape)}, eeg_feat shape: {tuple(eeg_feat.shape)}")
